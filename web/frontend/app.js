...
(function () {
  ...
  var OPEN_WORDS = ["aberto", "abrir", "open", "opened", "on", "unlock", "unlocked", "1", "true"];
  var CLOSED_WORDS = ["fechado", "fechar", "close", "closed", "off", "lock", "locked", "0", "false"];
  var PLATE_LABELS = ["license_plate", "plate", "car_plate", "licence_plate", "placa"];
  var CAR_LABELS = ["car", "automobile", "vehicle", "truck", "motorcycle", "bus", "carro"];
  ...
  var els = {};
  ["mqttPill", "videoPill", "clock", "demoBtn", "configBtn",
   "camLabel", "video", "mjpeg", "overlay", "playerWrapper", "noSignal", "gateRibbon", "recDot",
   "frigateHost", "cameraInput", "streamInput", "modeSelect", "videoBtn", "snapBtn",
   "boxesToggle", "beepToggle", "videoStatus",
   "gateBanner", "gateSub", "openBtn", "closeBtn",
   "personCard", "avatar", "personName", "personMeta", "personConf", "accessVerdict", "unknownVerdict",
   "plateText", "plateMeta", "eventLog", "mqttLog", "mqttCount",
   "exportBtn", "clearBtn", "clearMqttBtn", "configDialog", "configForm",
   "cfgBroker", "cfgWsPort", "cfgUser", "cfgPass", "cfgTopicStatus", "cfgTopicCommand",
   "cfgTopicFrigate", "cfgExtra", "cfgFrigatePort", "cfgAuth", "saveConfigBtn", "toasts"
  ...
  function authorizedList() {
    return String(cfg.authorized || "").split(/[,;\n]+/).map(function (s) { return s.trim().toLowerCase(); }).filter(Boolean);
  }
  ...
  function beep(freq, dur) {
    ...
    try {
      ...
      g.gain.value = 0.12;
      o.start();
      setTimeout(function () { o.stop(); ctx.close(); }, dur || 350);
    } catch (e) {}
  }

  // ---------- relógio ----------
  setInterval(function () { els.clock.textContent = hhmmss(); }, 1000);

  // ---------- portão ----------
  function interpretGateStatus(payload) {
    var text = String(payload == null ? "" : payload).trim().toLowerCase();
    if (text.charAt(0) === "{") {
      try {
        var data = JSON.parse(payload);
        var keys = ["status", "state", "portao", "gate", "door"];
        for (var i = 0; i < keys.length; i++) {
          if (data[keys[i]] !== undefined) { text = String(data[keys[i]]).trim().toLowerCase(); break; }
        }
      } catch (e) {}
    }
  ...
  function setGate(open, origin, silent) {
    ...
    if (open === true) {
      b.classList.add("open"); b.innerHTML = "PORTÃO<br/>ABERTO";
      els.gateRibbon.hidden = false;
      els.gateRibbon.textContent = "PORTÃO ABERTO";
    } else if (open === false) {
      b.classList.add("closed"); b.innerHTML = "PORTÃO<br/>FECHADO";
    } else {
      b.classList.add("unknown"); b.innerHTML = "PORTÃO<br/>—";
    }
    els.gateSub.textContent = "Último evento: " +
      (open === true ? "abertura" : open === false ? "fechamento" : "desconhecido") +
      " (" + origin + ") em " + hhmmss();
    if (!silent) {
      logEvent(open === true ? "🚪 Portão <b>ABERTO</b> (" + origin + ")" :
        open === false ? "🚪 Portão <b>FECHADO</b> (" + origin + ")" :
        "❓ Status desconhecido (" + origin + ")", open === true ? "ok" : "");
    }
  }
  function sendCommand(payload, label) {
    ...
    client.publish(cfg.topicCommand, String(payload), { qos: 1 }, function (err) {
      if (err) { toast("Falha ao publicar comando: " + err.message, "err"); return; }
      logEvent("👮 Porteiro: <b>" + label + "</b> → <span class='mono'>" +
        escapeHtml(cfg.topicCommand) + ": " + escapeHtml(String(payload)) + "</span>");
  ...
  function extractName(obj, fallbackBefore) {
    var raw = (obj && obj.sub_label !== undefined && obj.sub_label !== null)
      ? obj.sub_label
      : (fallbackBefore && fallbackBefore.sub_label);
    if (Array.isArray(raw)) {
      if (raw.length === 0) return { name: null, score: null };
      if (Array.isArray(raw[0])) return { name: raw[0][0] || null, score: raw[0][1] || null };
      // ["John Smith", 0.79]
      if (typeof raw[0] === "string" && (typeof raw[1] === "number" || raw[1] === undefined))
        return { name: raw[0], score: typeof raw[1] === "number" ? raw[1] : null };
      return { name: raw[0] != null ? String(raw[0]) : null, score: null };
    }
    if (typeof raw === "string" && raw.trim()) return { name: raw.trim(), score: null };
  ...
  function handleFrigatePayload(topic, text) {
    ...
    try { data = JSON.parse(text); } catch (e) {
      logEvent("📷 Evento Frigate (não-JSON) em <span class='mono'>" + escapeHtml(topic) + "</span>");
      return;
    }
    ...
    if (data.type === "lpr" || (data.plate && !data.after)) {
      ...
      if (plate) showPlate(String(plate).toUpperCase(), data.score || data.recognized_license_plate_score, data.camera);
      return;
    }
    ...
    var id = after.id || (before && before.id) || (after.label + "@" + (after.camera || ""));
    if (type === "end") {
      activeObjects.delete(id);
      return;
    }
    if (after.box && after.label) {
      activeObjects.set(id, { obj: after, before: before, lastSeen: Date.now() });
    }
    // Placa reconhecida acoplada ao carro (Frigate+): recognized_license_plate
    var recPlate = after.recognized_license_plate;
    if (recPlate) showPlate(String(recPlate).toUpperCase(), after.recognized_license_plate_score, after.camera);
    ...
    if (label === "person") {
      ...
      if (idn.name) {
        ...
        if (isAuthorized(idn.name)) {
          logEvent("✅ <b>" + escapeHtml(idn.name) + "</b> (autorizado, " +
            pct(score) + ") — abertura automática via Node-RED.", "ok");
          toast("Acesso liberado: " + idn.name, "ok");
          beep(880, 350);
        } else {
          logEvent("⚠️ <b>" + escapeHtml(idn.name) + "</b> NÃO está na lista de autorizados (" +
            pct(score) + ") — liberação manual disponível.", "warn");
          toast("Pessoa não autorizada: " + idn.name, "warn");
          beep(330, 500);
        }
      } else {
        showUnknown(after.score, after.camera);
        logEvent("👁 Pessoa detectada sem identificação (" + pct(after.score) + ") — aguardando reconhecimento.", "warn");
      }
    } else if (PLATE_LABELS.indexOf(label) !== -1) {
      var p = extractName(after, before);
      showPlate(p.name ? String(p.name).toUpperCase() : "(placa?)", p.score != null ? p.score : after.score, after.camera);
      ...
      if (s.name && /[A-Z]{2,3}[- ]?\d/.test(String(s.name).toUpperCase())) {
        showPlate(String(s.name).toUpperCase(), s.score != null ? s.score : after.score, after.camera);
      }
    }
  }
  ...
  function showPerson(name, score, camera, raw) {
    ...
    els.personMeta.textContent = "confiança " + pct(score) + " · " + (camera || "—") + " · " + hhmmss();
  ...
  function showUnknown(score, camera) {
    ...
    els.personMeta.textContent = "rosto não reconhecido · " + pct(score) + " · " + (camera || "—") + " · " + hhmmss();
    els.personConf.style.width = pct(score);
  ...
  function showPlate(plate, score, camera) {
    ...
    els.plateMeta.textContent = "confiança " + pct(score) + " · " + (camera || "—") + " · " + hhmmss();
    logEvent("🚗 Placa detectada: <b class='mono'>" + escapeHtml(plate) + "</b> (" + pct(score) + ")");
  }
  ...
  function connectMQTT() {
    ...
    var url = "ws://" + cfg.broker + ":" + cfg.wsPort;
    setMqttPill(false, "conectando…");
    logEvent("🔌 Conectando MQTT em <span class='mono'>" + escapeHtml(url) + "</span>…");
    var opts = { reconnectPeriod: 3000, connectTimeout: 8000, clean: true };
    if (cfg.username) { opts.username = cfg.username; opts.password = cfg.password || undefined; }
    ...
    client.on("connect", function () {
      ...
      if (cfg.topicFrigate.indexOf("#") === -1 && cfg.topicFrigate.indexOf("+") === -1) {
        subs.push(cfg.topicFrigate.replace(/\/$/, "") + "/#");
      }
      String(cfg.extraTopics || "").split(",").map(function (s) { return s.trim(); })
        .filter(Boolean).forEach(function (t) { subs.push(t); });
      subs.forEach(function (t) {
        client.subscribe(t, { qos: 0 }, function (err) {
          if (!err) logEvent("👂 Inscrito em <span class='mono'>" + escapeHtml(t) + "</span>");
        });
      });
      toast("MQTT conectado: " + url, "ok");
    });
    client.on("reconnect", function () { setMqttPill(false, "reconectando…"); });
    client.on("close", function () { setMqttPill(false, "desconectado"); });
    client.on("error", function (err) {
      setMqttPill(false, "erro");
      logEvent("❌ MQTT erro: " + escapeHtml(err && err.message || err), "warn");
    });
    client.on("message", function (topic, payload) {
      var text = "";
      try { text = payload.toString(); } catch (e) {}
      logMqtt(topic, text);
      if (topic === cfg.topicStatus) {
        var st = interpretGateStatus(text);
        if (st === null) {
          logEvent("❓ Status desconhecido em <span class='mono'>" + escapeHtml(topic) + "</span>: " + escapeHtml(text), "warn");
        } else {
          setGate(st, "mqtt");
          if (st) beep(660, 250);
        }
      ...
      if (frigBase && (topic === cfg.topicFrigate || topic.indexOf(frigBase) === 0)) {
  ...
  // ---------- vídeo + overlay ----------
  var video = els.video, mjpeg = els.mjpeg, canvas = els.overlay;
  var ctx = canvas.getContext("2d");

  function videoUrls() {
    var base = "http://" + cfg.frigateHost + ":" + cfg.frigatePort;
    return {
      mse: base + "/api/go2rtc/api/stream.mp4?src=" + encodeURIComponent(cfg.stream || cfg.camera),
  ...
  video.addEventListener("loadedmetadata", function () {
    ...
    els.videoStatus.textContent = "MSE " + video.videoWidth + "×" + video.videoHeight;
    logEvent("📹 Vídeo MSE conectado (" + video.videoWidth + "×" + video.videoHeight + ")", "ok");
  });
  video.addEventListener("error", function () {
    setVideoPill("erro", false);
    els.videoStatus.textContent = "erro no stream — confira IP/câmera e CORS do Frigate";
  });
  mjpeg.addEventListener("load", function () {
    ...
    els.videoStatus.textContent = "MJPEG " + mjpeg.naturalWidth + "×" + mjpeg.naturalHeight;
  });
  mjpeg.addEventListener("error", function () {
    mjpeg.style.display = "none";
    setVideoPill("erro", false);
    els.videoStatus.textContent = "MJPEG falhou — câmera offline ou nome incorreto";
  });
  ...
  function projectBox(box) {
    ...
    var cw = canvas.width || 1280, ch = canvas.height || 720;
    var vw = video.videoWidth || mjpeg.naturalWidth || cw;
    var vh = video.videoHeight || mjpeg.naturalHeight || ch;
    var allSmall = a.every(function (v) { return v >= 0 && v <= 1.5; });
    var x1, y1, x2, y2;
    if (allSmall) { // normalizado 0..1 (Double Take / atributos)
      x1 = a[0] * cw; y1 = a[1] * ch; x2 = a[2] * cw; y2 = a[3] * ch;
    } else { // pixels do frame de detecção → escala p/ canvas
      var sx = cw / vw, sy = ch / vh;
      x1 = a[0] * sx; y1 = a[1] * sy; x2 = a[2] * sx; y2 = a[3] * sy;
    }
    return { x: Math.min(x1, x2), y: Math.min(y1, y2), w: Math.abs(x2 - x1), h: Math.abs(y2 - y1) };
  }
  function styleFor(label, authorized) {
    label = String(label || "").toLowerCase();
    if (PLATE_LABELS.indexOf(label) !== -1) return { color: "#00e5ff", tag: "PLACA" };
    if (CAR_LABELS.indexOf(label) !== -1) return { color: "#42a5f5", tag: "VEÍCULO" };
    if (label === "person") return authorized
  ...
  function renderLoop() {
    ...
    activeObjects.forEach(function (entry, id) {
      if (entry.lastSeen < cutoff) { activeObjects.delete(id); return; }
      var obj = entry.obj;
      ...
      ctx.font = "bold " + Math.max(13, Math.round(canvas.width / 55)) + "px Arial";
      var tw = ctx.measureText(text).width + 12;
      var ty = Math.max(0, r.y - 26);
      ctx.fillStyle = st.color;
      ctx.fillRect(r.x, ty, Math.min(tw, canvas.width - r.x), 24);
      ctx.fillStyle = st.tag === "PESSOA?" || st.tag === "PLACA" ? "#111" : "#fff";
      ctx.fillText(text, r.x + 6, ty + 17);
    });
  }
  requestAnimationFrame(renderLoop);
  setInterval(function () { // limpeza periódica
    var cutoff = Date.now() - OBJECT_TTL_MS;
    activeObjects.forEach(function (e, id) { if (e.lastSeen < cutoff) activeObjects.delete(id); });
  }, 1500);
  ...
  function demoEvent() {
    ...
    if (pick < 0.45) {
      ...
      var obj = { id: id, camera: cfg.camera, label: "person", sub_label: [n, 0.9], score: 0.9, box: [420, 200, 620, 640] };
      activeObjects.set(id, { obj: obj, before: {}, lastSeen: Date.now() });
      showPerson(n, 0.92, cfg.camera);
      logEvent("✅ <b>" + escapeHtml(n) + "</b> (autorizado, 92%) — abertura automática (demo).", "ok");
      setGate(true, "demo");
      setTimeout(function () { setGate(false, "demo"); }, 5000);
    } else if (pick < 0.75) {
      var id2 = "demo-" + Date.now();
      activeObjects.set(id2, { obj: { id: id2, camera: cfg.camera, label: "person", score: 0.81, box: [700, 260, 880, 660] }, before: {}, lastSeen: Date.now() });
      showUnknown(0.81, cfg.camera);
      logEvent("⚠️ Pessoa <b>não identificada</b> (demo) — use ABRIR manualmente se for visita.", "warn");
    } else {
      var plate = ["ABC1D23", "QWE4F56", "XYZ9G88"][Math.floor(Math.random() * 3)];
      showPlate(plate, 0.95, cfg.camera);
      var id3 = "demo-" + Date.now();
      activeObjects.set(id3, { obj: { id: id3, camera: cfg.camera, label: "license_plate", sub_label: [plate, 0.95], score: 0.95, box: [760, 480, 980, 560] }, before: {}, lastSeen: Date.now() });
    }
  }
  ...
  function fillDialog() {
    els.cfgBroker.value = cfg.broker; els.cfgWsPort.value = cfg.wsPort;
    els.cfgUser.value = cfg.username || ""; els.cfgPass.value = cfg.password || "";
    els.cfgTopicStatus.value = cfg.topicStatus; els.cfgTopicCommand.value = cfg.topicCommand;
    els.cfgTopicFrigate.value = cfg.topicFrigate; els.cfgExtra.value = cfg.extraTopics || "";
    els.cfgFrigatePort.value = cfg.frigatePort; els.cfgAuth.value = cfg.authorized || "";
  }
  els.videoBtn.addEventListener("click", connectVideo);
  els.snapBtn.addEventListener("click", function () { window.open(videoUrls().snap, "_blank"); });
  els.openBtn.addEventListener("click", function () { sendCommand("1", "ABRIR"); });
  els.closeBtn.addEventListener("click", function () { sendCommand("0", "FECHAR"); });
  els.beepToggle.addEventListener("change", function () { cfg.beep = els.beepToggle.checked; saveConfig(); });
  els.clearBtn.addEventListener("click", function () { els.eventLog.innerHTML = ""; });
  els.clearMqttBtn.addEventListener("click", function () { els.mqttLog.innerHTML = ""; mqttSeen = 0; els.mqttCount.textContent = ""; });
  els.exportBtn.addEventListener("click", function () {
    var txt = Array.prototype.map.call(els.eventLog.children, function (li) { return li.textContent; }).join("\n");
    var blob = new Blob([txt], { type: "text/plain" });
    var a = document.createElement("a");
    a.href = URL.createObjectURL(blob); a.download = "portaria-log.txt"; a.click();
  });
  els.configBtn.addEventListener("click", function () { fillDialog(); els.configDialog.showModal(); });
  els.saveConfigBtn.addEventListener("click", function () {
    ...
    cfg.topicCommand = els.cfgTopicCommand.value.trim() || DEFAULTS.topicCommand;
    cfg.topicFrigate = els.cfgTopicFrigate.value.trim() || DEFAULTS.topicFrigate;
    ...
    saveConfig(); fillInputs();
    logEvent("⚙ Config salva. Reconectando MQTT…");
    connectMQTT();
  });
  els.demoBtn.addEventListener("click", function () {
    if (demoTimer) { clearInterval(demoTimer); demoTimer = null; els.demoBtn.textContent = "▶ Demo"; logEvent("⏹ Demo interrompido."); return; }
  ...
  (function () {
    try {
      ...
      ["broker", "wsPort", "frigateHost", "frigatePort", "camera", "stream"].forEach(function (k) {
  ...
  logEvent("👋 Painel iniciado. Câmera padrão <span class='mono'>" + escapeHtml(cfg.camera) + "</span>, stream <span class='mono'>" + escapeHtml(cfg.stream) + "</span>.");
